...
'action' DeclareNamespacedId(ID)
    ...
    'rule' DeclareNamespacedId(Id):
		...
		GetQualifiedName(DefiningId -> DefiningName)
		GetQualifiedName(Id -> QualifiedName)
		(|
			Id'Position -> Position

			IsNameEqualToName(QualifiedName, DefiningName)
			DefiningId'Position -> DefiningPosition
			Error_IdentifierPreviouslyDeclared(Position, Name, DefiningPosition)
		||
			(|
				Id'Position -> Position
				HasLocalMeaning(QualifiedName -> definingid(QualifiedDefiningId))
				QualifiedDefiningId'Position -> DefiningPosition
				Error_IdentifierPreviouslyDeclared(Position, QualifiedName, DefiningPosition)
			||
				-- Redeclare imported definition id, and declare
				-- this module's ID as normal
				DefineMeaning(DefiningName, definingid(DefiningId))
